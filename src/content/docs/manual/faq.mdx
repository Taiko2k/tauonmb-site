...
You can manually set the scaling Tauon uses by going `MENU` > `Settings...` > `View` and adjusting the slider.


:::tip
If album art tends to look too small, consider enabling the setting `Zoom album art to fit` under `Settings...` > `View`.

If text in the tracklist seems vertically off-centre, consider tweaking the setting `Text baseline` under `Settings...` > `Tracklist` to whatever looks visually best to you.
:::
